        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          <AnimatedSection disableHover={true}>
            ...
            <h2 className="text-3xl md:text-4xl font-bold text-white">
              ...
              <br />
              <span className="text-gold">and support</span>
            </h2>
            
            <p className="text-white/80 text-lg mt-4">
              Subscribe to our newsletter and be the first to know about new resources, 
          ...
          {/* Right Column - Resource Highlights */}
          <AnimatedSection direction="right" disableHover={true}>
            <div className="bg-white/5 backdrop-blur-sm rounded-3xl p-8 border border-white/10">
              <div className="flex items-center gap-3 mb-6">
                <BookOpen className="w-8 h-8 text-gold" />
                <h3 className="text-2xl font-bold text-white">Resource Highlights</h3>
              </div>
              
              <div className="space-y-4">
                {[
                  { icon: FileText, text: 'Free downloadable guides and toolkits', color: 'text-gold' },
                  { icon: Video, text: 'Video library with expert sessions', color: 'text-coral' },
                  { icon: LinkIcon, text: 'Curated community resource links', color: 'text-teal' },
                  { icon: Download, text: 'New resources added monthly', color: 'text-gold' },
                ].map((item, index) => (
                  <motion.div
                    key={index}
                    initial={{ opacity: 0, x: -20 }}
